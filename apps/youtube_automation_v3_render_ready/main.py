...
from services.video.video import VideoBuilderService
from services.upload.upload import UploaderService
...
async def lifespan(app: FastAPI):
    ...
    # Initialize services
    app.state.content_generator_service = ContentGeneratorService(
        llm_provider_name=settings.LLM_PROVIDER,
        llm_model_name=settings.LLM_MODEL,
        llm_api_key=settings.LLM_API_KEY
    )
    app.state.video_builder_service = VideoBuilderService(
        video_provider_name=settings.VIDEO_PROVIDER,
        video_api_key=settings.VIDEO_API_KEY
    )
    app.state.uploader_service = UploaderService(
        upload_provider_name=settings.UPLOAD_PROVIDER,
